...
Write exactly as a candidate would speak spontaneously to an interviewer, not as a resume, LinkedIn profile, article, documentation page, or AI assistant. Start directly. Natural conversational tone must come only from wording, contractions, and sentence flow, never from adding facts. Use the minimum factual claims necessary to answer. A short fully grounded answer is preferable to a richer answer containing reasonable assumptions.
...
class ResponseGuidanceService {
  ...
  buildGuidance(classification = {}, { continuity = null } = {}) {
    ...
    const lengthGuidance = classification.detailed
      ? 'The interviewer requested detail. Restate or clarify only facts explicitly present in the authorized evidence. Do not explain a purpose, mechanism, configuration, recipient, workflow, cause, impact, or outcome unless that exact kind of fact is supported. If evidence depth is exhausted, stop; a shorter answer is correct.'
      : 'Stay within the mode-specific range and keep the answer focused and easy to say aloud.';
    return [
      SHARED_GUIDANCE,
  ...
  getModeGuidance(classification) {
    switch (classification.mode) {
      ...
      case RESPONSE_MODES.TECHNICAL_CONCEPT:
        ...
        return 'Give a neutral, conversational technical explanation in about 60-120 words. Explain the idea the way one engineer would explain it aloud to another, using plain transitions and a compact example when useful. Avoid textbook headings, definition lists, and documentation-style prose. Do not force a first-person project narrative unless the question explicitly asks about the candidate\'s implementation and that experience is supported.';
