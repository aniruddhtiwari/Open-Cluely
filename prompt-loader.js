...
class PromptLoader {
  ...
  loadPrompts() {
	...
	/**
	 * Load all profile prompts from the profiles directory
	 */
  loadProfiles() {
	  ...
	  try {
		...
		for (const file of files) {
		  if (file.endsWith('.md')) {
			const profileName = path.basename(file, '.md').trim().toLowerCase();
			const filePath = path.join(profilesDir, file);
			const profileContent = fs.readFileSync(filePath, 'utf8');

			this.profiles.set(profileName, profileContent);
		  }
		}

    this.profilesLoaded = true;
  } catch (error) {
    console.error('Error loading profile prompts:', error);
    throw new Error(`Failed to load profile prompts: ${error.message}`);
  }
}

/**
 * Get the prompt content for a specific profile
 * @param {string} profileName - The profile name
 * @returns {string|null} The profile content or null if not found
 */
getProfilePrompt(profileName) {
  if (!this.profilesLoaded) {
    this.loadProfiles();
  }
  ...
  return this.profiles.get(normalizedProfileName) || null;
}
  ...
  getCombinedPrompt(skillName, profileName, programmingLanguage = null) {
	const skillPrompt = this.getSkillPrompt(skillName, programmingLanguage) || '';
	const profilePrompt = this.getProfilePrompt(profileName) || '';

	return [skillPrompt, profilePrompt]
    .filter(Boolean)
    .join('\n\n');
}
